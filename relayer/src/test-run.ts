...
const main = async () => {
	...
	const scrollEntrypointDeployment = await scrollEntrypointFactory.deploy(network1.router, gasPrice);
	...
	const baseEntrypointDeployment = await baseEntrypointFactory.deploy(network2.router, gasPrice);
